...
Note, this uses packages that are not imported by `parlitools`, namely `leaflet`, `htmltools` and `sf`.

```{r fig.width= 8, fig.height=4,message=FALSE}
  library(leaflet)
  library(sf)
  library(dplyr)
  devtools::install_github("evanodell/mnis") # If using v0.2.3 or less
